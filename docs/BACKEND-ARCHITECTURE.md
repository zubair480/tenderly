...
| Cause vocabulary | Adopt the frontend's ~10 interest chips (`food security, housing, youth, seniors, climate, health, disability inclusion, animal welfare, immigrant support, community safety`) as the seed data's `causes` vocabulary, since those exact strings arrive as `interests`. |
